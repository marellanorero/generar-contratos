...
import { getPartyGiro } from "@/core/entities/utils/getPartyGiro";
import { getPartyLegalRepresentative } from "@/core/entities/utils/getPartyLegalRepresentative";
import { getPartyNationality } from "@/core/entities/utils/getPartyNationality";
import { getPartyRutLegalRepresentative } from "@/core/entities/utils/getPartyRutLegalRepresentative";
...
export const CompanyContract: React.FC<Props> = ({ contract }) => {
    return (
        <div className="p-4 border rounded-xl shadow-md bg-white">
            <h2 className="text-xl font-bold mb-2 text-center">Contrato de Arrendamiento</h2>

            <p>
                En Santiago de Chile, a  <strong>{contract.createdAt.toLocaleDateString()}</strong>, comparecen: BG BODEGAS S.p.A., sociedad comercial del giro de Arriendo de Bodegas, Rol Único Tributario 77.365.227-0, representada según se acreditará por don GERMAN ALBERTO ROJAS GONZALEZ, chileno, soltero, ingeniero civil en sistemas, cédula nacional de identidad número 14.703.000-2 y  don BERNARDO MARCELO ARELLANO  ASTUDILLO,  chileno,  divorciado,  ingeniero  comercial, cédula nacional de identidad número 6.872.948- 3, todos domiciliados en calle Exequiel Fernández Nº2301, comuna de Macul, Santiago, en  adelante  “La arrendadora” o “la parte arrendadora”,  y  <strong>{contract.party.name}</strong>, sociedad del giro de <strong>{getPartyGiro(contract.party)}</strong>, Rol Único Tributario N° <strong>{getPartyRut(contract.party)}</strong> representada por don/ña <strong>{getPartyLegalRepresentative(contract.party)}</strong>,  <strong>{getPartyNationality(contract.party)}</strong>,  cédula  nacional  de identidad número <strong>{getPartyRutLegalRepresentative(contract.party)}</strong>,  ambas  domiciliadas  en  <strong>{contract.party.address}</strong>,  comuna  de  <strong>{contract.party.comuna}</strong>, en adelante  “la parte arrendataria”, todos los comparecientes mayores de edad quienes acreditan su identidad con las cédulas mencionadas y exponen:

            </p>
            <br></br>
            <p>
                <strong className="subtitulo">PRIMERO: </strong> Por este acto, BG BODEGAS S.p.A. sociedad comercial del giro de Arriendo de Bodegas debidamente representada, declara Ser la  sub  arrendataria  de la propiedad  ubicada  en  calle Exequiel Fernández Nº2301 y 2321, de la comuna de Macul, según consta en contrato de arrendamiento celebrado con Servicios Consultorías y Operaciones Logísticas Benar Limitada, con  fecha 14 de marzo de 2014, ante el Notario Público de Santiago don Felix Jara Cadot, acuerdo de voluntades que en su cláusula novena otorga de manera expresa a la arrendataria la facultad de sub arrendar..

            </p>
            <br></br>
            <p>
                <strong className="subtitulo">SEGUNDO: </strong> Por este acto, BG BODEGAS S.p.A., sociedad comercial del giro de Arriendo de Bodegas debidamente representada da en arrendamiento a <strong>{contract.party.name}</strong> , Rol Único Tributario <strong>{getPartyRut(contract.party)}</strong>, quien, debidamente representada, acepta parte de la propiedad ubicada en Exequiel Fernández N°2301 Bodega {contract.warehouse.number} de la comuna de Macul, identificada en el plano adjunto y que pasa a formar parte integrante del presente Contrato de Arrendamiento, de aproximadamente {contract.warehouse.capacity} metros cuadrados ( {getCapacityString(contract.warehouse.capacity)} ), en adelante e indistintamente referido como el “inmueble”.
